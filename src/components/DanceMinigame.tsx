...
import { Activity, ArrowUp, ArrowDown, ArrowLeft, ArrowRight } from "lucide-react";
...
export function DanceMinigame({ 
  ...
  const generateSequence = () => {
    ...
    setStatus("showing");
    const newSeq = Array.from({ length: round + 2 }, () => DANCE_MOVES[Math.floor(Math.random() * DANCE_MOVES.length)]);
    setSequence(newSeq);
    setPlayerSequence([]);
  ...
  return (
    <div className="bg-slate-950 p-8 rounded-3xl border border-emerald-500/30 text-center w-full max-w-md mx-auto shadow-[0_0_50px_rgba(16,185,129,0.2)] select-none">
      ...
      <p className="text-emerald-200/70 mb-6 text-sm">Gösterilen yönleri sırasıyla tekrarla. (3 Tur)</p>
      ...
      <div className="h-24 bg-slate-900 rounded-xl mb-8 flex items-center justify-center border border-white/5 relative overflow-hidden">
        {status === "showing" ? (
          ...
          <span className="text-rose-500 font-bold text-xl">AYAKLARIN BİRBİRİNE DOLANDI!❌</span>
        ) : status === "success" ? (
          <span className="text-emerald-500 font-bold text-xl">MÜKEMMEL HAREKET! ✨</span>
        ) : (
          <div className="flex gap-2">
            {playerSequence.map((dir, i) => (
              <div key={i} className="text-emerald-300 bg-emerald-900/30 p-2 rounded-lg">
                {getArrowIcon(dir)}
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-3 gap-2 w-48 mx-auto">
        <div />
        <button onClick={() => handleArrowPress("UP")} disabled={status !== "playing"} className="bg-slate-800 hover:bg-emerald-900/50 p-4 rounded-xl flex items-center justify-center border border-slate-700 active:bg-emerald-600 transition-colors">
          <ArrowUp size={24} className="text-white" />
        </button>
        <div />
        <button onClick={() => handleArrowPress("LEFT")} disabled={status !== "playing"} className="bg-slate-800 hover:bg-emerald-900/50 p-4 rounded-xl flex items-center justify-center border border-slate-700 active:bg-emerald-600 transition-colors">
          <ArrowLeft size={24} className="text-white" />
        </button>
        <button onClick={() => handleArrowPress("DOWN")} disabled={status !== "playing"} className="bg-slate-800 hover:bg-emerald-900/50 p-4 rounded-xl flex items-center justify-center border border-slate-700 active:bg-emerald-600 transition-colors">
          <ArrowDown size={24} className="text-white" />
        </button>
        <button onClick={() => handleArrowPress("RIGHT")} disabled={status !== "playing"} className="bg-slate-800 hover:bg-emerald-900/50 p-4 rounded-xl flex items-center justify-center border border-slate-700 active:bg-emerald-600 transition-colors">
